 ...
 *
 * Pedro entra al ERP en producción y ve sus módulos Niveles y
 * Contratos vacíos (el demoData del sector pack solo se aplica al
 * provisionar un tenant nuevo, no a los que ya existían). Este
 * endpoint los siembra idempotentemente:
 *
 *   - 11 Niveles según TEST 19 (M1..M4 x cuota/horas, A-A1, B-B15, B-B20)
 *   - N Contratos básicos, uno por cliente existente en el tenant,
 *     con valores razonables (Mantenimiento Nivel 1 mensual por
 *     defecto; el operador puede editar después).
 *
 * Tolerante: si los Niveles ya existen, no los duplica. Si ya hay
 * contratos, no toca nada de contratos (solo siembra si la tabla
 * está vacía).
 *
...
const NIVELES_SEED: Array<Record<string, string>> = [
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2) Contratos: solo siembro si la tabla está vacía, para no
    //    pisar trabajos que el operador haya empezado a editar.
    ...
    if ((contratosExistentes as Array<Record<string, string>>).length === 0) {
      ...
      // Asignación por defecto: Mant M1 mensual al primer cliente,
      // Mant M2 mensual al segundo, Bono B15 al tercero, Acuerdo A1
      // trimestral al cuarto. Si hay más, todos Mant M1.
    ...
    return NextResponse.json({
      ...
      mensaje:
        nivelesCreados + " niveles creados (de 11 totales). " +
        contratosCreados + " contratos creados (uno por cliente del tenant).",
    });
  } catch (e) {
