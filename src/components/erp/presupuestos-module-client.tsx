...
type PresupuestosModuleClientProps = {
  runtimeInfo: RuntimeClientInfo;
};

const MODULE_KEY = "presupuestos";

export default function PresupuestosModuleClient(props: PresupuestosModuleClientProps) {
  ...
  async function handleDelete(row: ModuleRecord) {
    const confirmed = window.confirm("¿Seguro que quieres borrar este presupuesto?");
    if (!confirmed) {
      return;
    }
    ...
    <main style={{ padding: 24, fontFamily: "Arial, sans-serif", maxWidth: 1300, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 32, marginBottom: 8 }}>
          {schema?.title || "Presupuestos"}
        </h1>
        <p style={{ marginBottom: 8 }}>
          Gestión real de presupuestos del ERP activo.
        </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.9fr 1fr", gap: 16, alignItems: "start" }}>
        <section style={{ display: "grid", gap: 16 }}>
          ...
          >
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <input
                ...
                placeholder="Buscar por número, cliente, concepto, estado o importe..."
                style={{
                  flex: "1 1 320px",
          ...
          <ModuleGrid
            title={isLoading ? "Cargando..." : "Presupuestos"}
            rows={filteredRows}
            columns={columns}
            emptyText="Todavía no hay presupuestos registrados."
        ...
        <aside style={{ display: "grid", gap: 16 }}>
          ...
          <section style={{ border: "1px solid #ddd", borderRadius: 16, background: "#fff", padding: 16 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              <li>Relacionar presupuestos con clientes reales.</li>
              <li>Editar estado, importe y validez.</li>
      ...
      {schema ? (
        <ModuleForm
          title={editingRow ? "Editar presupuesto" : "Nuevo presupuesto"}
          fields={schema.fields}
          initialValues={initialValues}
          submitLabel={editingRow ? "Guardar cambios" : "Crear presupuesto"}
          isOpen={isModalOpen}
          isSaving={isSaving}
          errorText={errorText}
          onClose={() => {
