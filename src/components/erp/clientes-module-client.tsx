...
type ClientesModuleClientProps = {
  runtimeInfo: RuntimeClientInfo;
};

const MODULE_KEY = "clientes";
...
export default function ClientesModuleClient(props: ClientesModuleClientProps) {
  ...
  async function loadRows() {
    ...
    try {
      ...
      const nextSchema: ModuleSchema = {
        ...data.schema,
        title: labels.clientModuleTitle,
        fields: data.schema.fields.map((field) => {
          if (field.key === "nombre") {
            return {
              ...field,
              label: labels.clientSingular.charAt(0).toUpperCase() + labels.clientSingular.slice(1),
              placeholder:
                runtimeInfo.businessType === "clinica-dental"
                  ? "Ejemplo: Laura Martín"
                  : field.placeholder,
            };
          }
          ...
          if (field.key === "notas") {
            return {
              ...
              placeholder:
                runtimeInfo.businessType === "clinica-dental"
                  ? "Observaciones clínicas o comerciales del paciente"
                  : field.placeholder,
            };
          }
  ...
  async function handleDelete(row: ModuleRecord) {
    const confirmed = window.confirm(
      "¿Seguro que quieres borrar este " + labels.clientSingular + "?"
    );
    if (!confirmed) {
      return;
    }
  ...
  const columns = [
    { key: "nombre", label: labels.clientModuleTitle.slice(0, -1) || labels.clientModuleTitle },
    { key: "segmento", label: runtimeInfo.businessType === "clinica-dental" ? "Tipo" : "Segmento" },
    {
      key: "estado",
      label: "Estado",
    ...
    <main style={{ padding: 24, fontFamily: "Arial, sans-serif", maxWidth: 1300, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 32, marginBottom: 8 }}>
          {labels.clientModuleTitle}
        </h1>
        <p style={{ marginBottom: 8 }}>
          Gestión real de {labels.clientPlural} del ERP activo.
        </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.9fr 1fr", gap: 16, alignItems: "start" }}>
        <section style={{ display: "grid", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <input
                ...
                placeholder={
                  ...
                  ", email, tipo/segmento, estado o teléfono..."
                }
                style={{
                  flex: "1 1 320px",
          ...
          <ModuleGrid
            title={isLoading ? "Cargando..." : labels.clientModuleTitle}
            rows={filteredRows}
            columns={columns}
            emptyText={"Todavía no hay " + labels.clientPlural + " registrados."}
        ...
        <aside style={{ display: "grid", gap: 16 }}>
          ...
          <section style={{ border: "1px solid #ddd", borderRadius: 16, background: "#fff", padding: 16 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              <li>Crear y editar {labels.clientPlural} reales.</li>
              <li>Buscar sobre la rejilla.</li>
              <li>Borrar registros.</li>
              <li>Abrir ficha 360 de cada {labels.clientSingular}.</li>
            </ul>
          </section>
        </aside>
      </div>
