...
type CitasModuleClientProps = {
  runtimeInfo: RuntimeClientInfo;
};

const MODULE_KEY = "citas";

export default function CitasModuleClient(props: CitasModuleClientProps) {
  const { runtimeInfo } = props;

  const [schema, setSchema] = useState<ModuleSchema | null>(null);
  const [rows, setRows] = useState<ModuleRecord[]>([]);
  const [pacienteOptions, setPacienteOptions] = useState<OptionItem[]>([]);
  ...
  async function loadPacienteOptions() {
    try {
      ...
      setPacienteOptions(Array.isArray(data.options) ? data.options : []);
    } catch {
      setPacienteOptions([]);
    }
  }
    ...
    <main style={{ padding: 24, fontFamily: "Arial, sans-serif", maxWidth: 1300, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ marginBottom: 8 }}>
          Gestión real de citas del ERP activo.
        </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.9fr 1fr", gap: 16, alignItems: "start" }}>
        <section style={{ display: "grid", gap: 16 }}>
          ...
          >
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <input
                ...
                placeholder="Buscar por paciente, profesional, tipo, estado, fecha u hora..."
                style={{
                  flex: "1 1 320px",
          ...
          <ModuleGrid
            title={isLoading ? "Cargando..." : "Citas"}
            rows={filteredRows}
            columns={columns}
            emptyText="Todavía no hay citas registradas."
        ...
        <aside style={{ display: "grid", gap: 16 }}>
          <section style={{ border: "1px solid #ddd", borderRadius: 16, background: "#fff", padding: 16 }}>
            ...
            <p><strong>Pacientes disponibles:</strong> {pacienteOptions.length}</p>
          ...
          <section style={{ border: "1px solid #ddd", borderRadius: 16, background: "#fff", padding: 16 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              <li>Relacionar citas con pacientes reales.</li>
              <li>Editar profesional, tipo, estado y horario.</li>
      ...
      {schema ? (
        <ModuleForm
          title={editingRow ? "Editar cita" : "Nueva cita"}
          fields={schema.fields}
          initialValues={initialValues}
          submitLabel={editingRow ? "Guardar cambios" : "Crear cita"}
          isOpen={isModalOpen}
          isSaving={isSaving}
          errorText={errorText}
          onClose={() => {
