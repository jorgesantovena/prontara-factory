...
type FacturacionModuleClientProps = {
  runtimeInfo: RuntimeClientInfo;
};

const MODULE_KEY = "facturacion";

export default function FacturacionModuleClient(props: FacturacionModuleClientProps) {
  ...
  const [presupuestoOptions, setPresupuestoOptions] = useState<OptionItem[]>([]);
  ...
  async function loadPresupuestoOptions() {
    try {
      const response = await fetch("/api/erp/options?module=presupuestos", {
        cache: "no-store",
      });
      const data = (await response.json()) as OptionsResponse;
      ...
      setPresupuestoOptions([{ value: "", label: "Sin presupuesto relacionado" }, ...options]);
    } catch {
      setPresupuestoOptions([{ value: "", label: "Sin presupuesto relacionado" }]);
    }
  }
  ...
  useEffect(() => {
    ...
    void loadPresupuestoOptions();
  }, []);

  useEffect(() => {
    void loadRows();
  }, [clienteOptions.length, presupuestoOptions.length]);
  ...
  async function handleDelete(row: ModuleRecord) {
    const confirmed = window.confirm("¿Seguro que quieres borrar esta factura?");
    if (!confirmed) {
      return;
    }
    ...
    <main style={{ padding: 24, fontFamily: "Arial, sans-serif", maxWidth: 1300, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 32, marginBottom: 8 }}>
          {schema?.title || "Facturación"}
        </h1>
        <p style={{ marginBottom: 8 }}>
          Gestión real de facturas del ERP activo.
        </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.9fr 1fr", gap: 16, alignItems: "start" }}>
        <section style={{ display: "grid", gap: 16 }}>
          ...
          >
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <input
                ...
                placeholder="Buscar por número, cliente, presupuesto, estado o importe..."
                style={{
                  flex: "1 1 320px",
          ...
          <ModuleGrid
            title={isLoading ? "Cargando..." : "Facturación"}
            rows={filteredRows}
            columns={columns}
            emptyText="Todavía no hay facturas registradas."
        ...
        <aside style={{ display: "grid", gap: 16 }}>
          <section style={{ border: "1px solid #ddd", borderRadius: 16, background: "#fff", padding: 16 }}>
            ...
            <p><strong>Presupuestos disponibles:</strong> {presupuestoOptions.length - 1}</p>
          ...
          <section style={{ border: "1px solid #ddd", borderRadius: 16, background: "#fff", padding: 16 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              <li>Relacionar facturas con clientes reales.</li>
              <li>Relacionar facturas con presupuestos existentes.</li>
              <li>Editar estado, importe y vencimiento.</li>
      ...
      {schema ? (
        <ModuleForm
          title={editingRow ? "Editar factura" : "Nueva factura"}
          fields={schema.fields}
          initialValues={initialValues}
          submitLabel={editingRow ? "Guardar cambios" : "Crear factura"}
          isOpen={isModalOpen}
          isSaving={isSaving}
          errorText={errorText}
          onClose={() => {
