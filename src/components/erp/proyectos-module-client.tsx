...
type ProyectosModuleClientProps = {
  runtimeInfo: RuntimeClientInfo;
};

const MODULE_KEY = "proyectos";

export default function ProyectosModuleClient(props: ProyectosModuleClientProps) {
  ...
  async function handleDelete(row: ModuleRecord) {
    const confirmed = window.confirm("¿Seguro que quieres borrar este proyecto?");
    if (!confirmed) {
      return;
    }
    ...
    <main style={{ padding: 24, fontFamily: "Arial, sans-serif", maxWidth: 1300, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 32, marginBottom: 8 }}>
          {schema?.title || "Proyectos"}
        </h1>
        <p style={{ marginBottom: 8 }}>
          Gestión real de proyectos del ERP activo.
        </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.9fr 1fr", gap: 16, alignItems: "start" }}>
        <section style={{ display: "grid", gap: 16 }}>
          ...
          >
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <input
                ...
                placeholder="Buscar por proyecto, cliente, responsable, estado o prioridad..."
                style={{
                  flex: "1 1 320px",
          ...
          <ModuleGrid
            title={isLoading ? "Cargando..." : "Proyectos"}
            rows={filteredRows}
            columns={columns}
            emptyText="Todavía no hay proyectos registrados."
        ...
        <aside style={{ display: "grid", gap: 16 }}>
          ...
          <section style={{ border: "1px solid #ddd", borderRadius: 16, background: "#fff", padding: 16 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              <li>Relacionar proyectos con clientes reales.</li>
              <li>Editar fechas, responsable, prioridad y estado.</li>
      ...
      {schema ? (
        <ModuleForm
          ...
          submitLabel={editingRow ? "Guardar cambios" : "Crear proyecto"}
          isOpen={isModalOpen}
          isSaving={isSaving}
          errorText={errorText}
          onClose={() => {
